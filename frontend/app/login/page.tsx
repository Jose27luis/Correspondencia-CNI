...
export default function PaginaLogin() {
  ...
  return (
    <main className="grid min-h-screen lg:grid-cols-[1.05fr_1fr]">
      <section className="relative hidden overflow-hidden bg-zinc-950 px-12 py-14 text-zinc-100 lg:flex lg:flex-col lg:justify-between">
        <motion.div
          aria-hidden="true"
          className="pointer-events-none absolute -right-24 -top-24 h-[26rem] w-[26rem] rounded-full bg-emerald-500/15 blur-3xl"
          animate={animacionReducida ? undefined : { scale: [1, 1.12, 1], opacity: [0.5, 0.8, 0.5] }}
          transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          aria-hidden="true"
          className="pointer-events-none absolute -bottom-32 -left-20 h-[22rem] w-[22rem] rounded-full bg-sky-500/10 blur-3xl"
          animate={animacionReducida ? undefined : { scale: [1, 1.18, 1], opacity: [0.4, 0.7, 0.4] }}
          transition={{ duration: 15, repeat: Infinity, ease: "easeInOut", delay: 1.5 }}
        />

        <motion.div className="relative" variants={contenedor} initial="oculto" animate="visible">
          <motion.span
            variants={elemento}
          ...
          <motion.h1 variants={elemento} className="mt-8 text-3xl font-semibold leading-tight">
            Correspondencia
        ...
        >
          {ventajas.map((ventaja) => (
            <motion.div key={ventaja.titulo} variants={elemento}>
              <dt className="text-xs uppercase tracking-wide text-zinc-400">{ventaja.titulo}</dt>
          ...
          <form onSubmit={handleSubmit(enviar)} className="mt-8 space-y-5" noValidate>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                className={cn(errors.correo && "border-destructive focus-visible:ring-destructive/30")}
                {...register("correo")}
              />
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <Input
                  ...
                  className={cn(
                    "pr-11",
                    errors.contrasena && "border-destructive focus-visible:ring-destructive/30",
                  )}
                  {...register("contrasena")}
                />
                <Button
            ...
            <AnimatePresence initial={false}>
              {errorGeneral && (
                ...
                >
                  <motion.div
                    animate={animacionReducida ? undefined : { x: [0, -6, 6, -4, 4, 0] }}
